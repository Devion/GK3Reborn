...
|---|---|---|---|
| FFmpeg (LGPL shared build) | 7.1 | LGPL-2.1-or-later | https://github.com/FFmpeg/FFmpeg (tag `n7.1`) |
...
FFmpeg and MoltenVK are fetched by `build/fetch-native.sh`, which pins the exact upstream
release and verifies the SHA-256 of every archive it downloads. The rest arrive as NuGet
native packages and are relocated into `libs/<rid>/` by the targets in
`src/GK3Reborn.Host/GK3Reborn.Host.csproj`.

### The two LGPL ones

FFmpeg and OpenAL Soft are LGPL. Both are shipped as unmodified shared libraries loaded at
run time, which is the arrangement the LGPL is written for: the terms are satisfied by
saying so, by not linking them statically, and by leaving them replaceable — delete either
file and drop in your own build of the same generation and the game will use it.

Their corresponding source is at the repositories above, offered from the same kind of
public location as the binaries themselves.

## Managed dependencies

| Component | Version | Licence |
|---|---|---|
| FFMediaToolkit | 4.8.1 | MIT |
| NLayer | 2.0.1 | MIT |
