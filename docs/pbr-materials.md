...
## Where the classifier is wrong, and what is done about it

A classifier reads a texture and guesses what the surface is. It is right about most of
6,657 of them and it is wrong in two ways that are worth naming, because both were reported
as rendering faults and neither was one.

**A photograph of worn stone under a 1999 lightmap looks like concrete.** The church's nave
came out at 0.90 roughness and the threshold for a reflection being worth tracing at all is
0.60, so nothing in the nave was ever asked for one — reported as "the tile floor in the
hotel, the tile floor in the church don't reflect much at all". Sixteen floors are corrected
in `tools/rooms/polished-floors.py`: the church's nave and its tiling, the hotel's glazed
tile and its waxed boards, the kitchen and bathroom tiling, and marble, which the pass had
at 0.72 — that is limestone.

**A face has its shading painted into it.** Generated from the colour texture, a face's
normal map turns the cheekbone and the blush into relief, and comes out with fully
saturated lobes across both cheeks. That reads as a faceted grey smear at any lighting
quality and was reported as shadow acne. All 86 face and forehead materials are corrected
to `normalStrength` 0.25 — turned down rather than off, because the eyes, the nostrils and
the lips are real relief and survive a quarter strength while the painted shading stops
being geometry.

**An edit also marks the material authored, and that is half of why it works.** Where a
surface has a generated ORM map the map is normally the answer, being a measurement of that
surface rather than a guess at it. A correction somebody made after looking at the room
in-engine outranks both — without that, the same pass that got it wrong would overwrite the
correction. See `SurfaceFinish` and ADR 0006.
