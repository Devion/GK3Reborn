...
public enum FrontEndOutcome
{
    ...
    /// <summary>Let go of whatever the story is holding and give the room back to the player.</summary>
    Unstick,
}

/// <summary>The menu in front of the game: what each page holds and what choosing a row does.</summary>
public sealed class FrontEnd
{
    ...
    [
        UpscalerQuality.Native, UpscalerQuality.UltraQuality, UpscalerQuality.Quality, UpscalerQuality.Balanced, UpscalerQuality.Performance,
        UpscalerQuality.UltraPerformance, ];

    /// <summary>The frame-generation settings this machine can actually reach.</summary>
    private FrameGeneration[] Generations => FrameGenerationMaximum <= 0 ? [.. FrameGenerations.All]
    ...
    /// <summary>The sizes the display page offers, plus whatever the monitor's own is.</summary>
    private static readonly (int Width, int Height)[] Sizes =
    [
        (0, 0), (1280, 720), (1600, 900), (1920, 1080), (2560, 1440), (3440, 1440), (3840, 2160), ];

    /// <summary>The port's own words, in the language the game is being played in.</summary>
    ...
    /// <summary>Whether the game's own title art is on screen behind the menu.</summary>
    public bool Illustrated { get; set; }

    /// <summary>Whether the port's own title screen could be drawn at all on this installation.</summary>
    public bool ModernMenuAvailable { get; set; }

    /// <summary>The languages this installation can actually be played in.</summary>
    ...
    /// <summary>The settings screen's sections, in the order they are listed down its side.</summary>
    public static IReadOnlyList<MenuSection> Sections { get; } =
    [
        new("gameplay", "General"), new("video", "Picture"), new("display", "Display"), new("audio", "Sound"), new("controls", "Controls"), ];

    /// <summary>The same sections, named in the player's own language.</summary>
    ...
    public bool StepSection(int by)
    {
        ...
        int next = ((at + by) % SectionPages.Length + SectionPages.Length) % SectionPages.Length;
    ...
    public string Title => Page switch
    {
        ...
        FrontEndPage.Main => InGame ? Text.Say("menu.title.paused", "Paused") : Illustrated ? string.Empty : "Gabriel Knight 3",
        FrontEndPage.Save => Text.Say("menu.title.save", "Save Game"), FrontEndPage.Load => Text.Say("menu.title.load", "Restore Game"),
    ...
    public FrontEndOutcome Choose(MenuAction action)
    {
        ...
        switch (action.Id)
        {
            ...
            // The settings screen, opened at whichever section was last looked at.
            case "options":
                Page = _lastSection;
                return FrontEndOutcome.Stay;
            ...
            default:
                ...
                // A row on the Controls page, which is not a setting to be stepped but a question to be answered by pressing something.
                if (action.Id.StartsWith("key:", StringComparison.Ordinal) || action.Id.StartsWith("pad:", StringComparison.Ordinal) ||
    ...
    public bool Back()
    {
        // Listening for a key is a state to get out of, and Escape is what everybody will press to do it.
        if (Listening)
        {
            Cancel();
        ...
        // Out of whatever is showing and back to the top.
        if (Section >= 0)
        {
            _lastSection = Page;
        }
    ...
    /// <summary>Where these settings came from, and where they go back to.</summary>
    ...
    /// <summary>A row that is on or off, reading in the player's own language.</summary>
    ...
    private MenuItem Toggle(string id, string text, bool on) => MenuItem.Toggle(id, text, on) with
        {
    ...
    private IReadOnlyList<MenuItem> Main() => InGame
        ...
        // Paused.
        ? [ MenuItem.Button("resume", Text.Say("menu.resume", "Resume")), MenuItem.Button("save", Text.Say("menu.save", "Save")),
            MenuItem.Button("load", Text.Say("menu.load", "Restore")),

            // A room can wedge: an approach walk far longer than anybody will sit through, a script that parked and never came back, a clip on the.
            MenuItem.Button("unstick", Text.Say("menu.unstick", "Get Unstuck")),

            MenuItem.Button("options", Text.Say("menu.options", "Settings")), MenuItem.Button("quit", Text.Say("menu.leave", "Leave the Game")), ]

        // The original's own five, in its own order.
        : [ MenuItem.Button("intro", Text.Say("menu.intro", "Intro")), MenuItem.Button("play", Text.Say("menu.play", "Play")),
    ...
    private string Described(string slot)
    {
        ...
        {
            SaveStore.QuickSlot => Text.Say("save.quick", "Quick save"), SaveStore.AutoSlot => Text.Say("save.auto", "Autosave"),
            _ when IsNumbered(slot) => Text.Say("save.slot", "Slot {0}", slot.TrimStart('0')),

            // A game the 1999 original wrote, brought across under its own file name.
            _ when slot.StartsWith("gk3-", StringComparison.OrdinalIgnoreCase) => Text.Say("save.original", "Original save"),
        ...
        // Trimmed, because the panel is as wide as its widest row and a window is only so wide.
        if (called.Length > 28)
        {
            called = called[..27].TrimEnd() + "\u2026";
        }

        return string.Create( CultureInfo.InvariantCulture, $"{name}  -  {called}  -  {save.Written.LocalDateTime:dd/MM HH:mm}");
    }

    /// <summary>Which of the vendors' runtimes are installed, for the rows that need to say.</summary>
    ...
    /// <summary>Which upscalers this machine may be offered.</summary>
    ...
    /// <summary>How many frames the runtime will generate for each drawn one, or nought for none.</summary>
    ...
    /// <summary>Everything about what is drawn: how it is lit, what it is built from, and how it is scaled up to the window.</summary>
    private List<MenuItem> Video()
    {
        List<MenuItem> rows =
        [
            MenuItem.Heading(Text.Say("picture.lighting", "Lighting")), MenuItem.Choice(
                "picture", Text.Say("picture.lighting", "Lighting"), Describe(Settings.Picture)),

            // Dead with no rays, because there is nothing for it to take away then: the bake is the room's lighting at that tier and the rig only.
            Toggle( "realistic", Text.Say("picture.realistic", "Only real light sources"), Settings.RealisticLighting) with
            {
        ...
        {
            // What the player cannot find out by trying it in one room: the rooms this changes most are the ones the artists were propping up.
            rows.Add(MenuItem.Label(Text.Say( "picture.realistic.note", "The artists' fills, ambients and bounces are switched off. Rooms lit "
        ...
        [
            ...
            Toggle( "floorreflect", Text.Say("picture.floorreflect", "Floors reflect the room"), Settings.FloorReflections),

            // A multiplier rather than a percentage, because one is the physical answer and the row is about departing from it.
            MenuItem.Slider( "reflectivity", Text.Say("picture.reflectivity", "How strongly"),
                Settings.Reflectivity / GK3Reborn.Game.Settings.MostReflective, string.Create(
                    CultureInfo.InvariantCulture, $"{Settings.Reflectivity:F1}x")),

            MenuItem.Heading(Text.Say("picture.detail", "Detail")),

            Toggle( "enhanced", Text.Say("picture.enhanced", "Higher-resolution textures"), Settings.EnhancedTextures),

            Toggle( "trees", Text.Say("picture.trees", "Modelled trees"), Settings.ModelledTrees),

            Toggle( "grass", Text.Say("picture.grass", "Grass on the lawns"), Settings.Grass),

            Toggle( "terrain", Text.Say("picture.terrain", "Reconstructed horizon"), Settings.TerrainBackdrop),

            Toggle( "rooms", Text.Say("picture.rooms", "Rounded room objects"), Settings.ImprovedSceneGeometry),

            Toggle( "rails", Text.Say("picture.rails", "Solid railings and fences"), Settings.ThickCutoutCards),

            Toggle( "birds", Text.Say("picture.birds", "Birds in the sky"), Settings.Birds),

            Toggle( "insects", Text.Say("picture.insects", "Insects and dust in the air"), Settings.InsectsAndDust),

            Toggle( "sunrays", Text.Say("picture.sunrays", "Rays of sunlight"), Settings.SunRays),

            Toggle( "shimmer", Text.Say("picture.shimmer", "Heat haze in the distance"), Settings.HeatHaze),

            // One row for two towns.
            Toggle( "towns", Text.Say("picture.towns", "Rebuilt towns"), Settings.RebuiltTowns),

            // The one thing in this group a player cannot see for themselves: the room standing round them was built from whichever set was chosen.
            MenuItem.Label( Text.Say("picture.detail.note", "These take effect at the next door.")), ]);
    ...
    /// <summary>The window, the monitor, and how bright the display is allowed to go.</summary>
    private List<MenuItem> Display()
    {
        List<MenuItem> rows = [];

        // Windows only, because it is the only machine where there is a choice: every other one has Vulkan and nothing else, and a row with one.
        if (RenderBackends.IsPossible(RenderBackend.Direct3D12))
        {
            rows.Add(MenuItem.Choice( "backend", Text.Say("display.backend", "Graphics API"), DescribeBackend()));
        }

        rows.AddRange(
        [
            MenuItem.Choice( "window", Text.Say("display.window", "Window"), Describe(Settings.Display)),

            // Dead rather than explained.
            ...
            MenuItem.Slider( "textsize", Text.Say("display.textsize", "Text size"), Fraction(Settings.TextScale, SmallestText, LargestText),
                DescribeTextScale()),

            Toggle( "modernmenu", Text.Say("display.modernmenu", "Title screen"), Settings.ModernMenu) with
            {
                Enabled = ModernMenuAvailable,

                // Named for what it shows rather than for which of the two it is.
                Value = Settings.ModernMenu && ModernMenuAvailable ? Text.Say("display.modernmenu.new", "The port's own")
                    : Text.Say("display.modernmenu.old", "The original"),
            },

            Toggle( "vsync", Text.Say("display.vsync", "Wait for the display"), Settings.VerticalSync),

            Toggle( "hdr", Text.Say("display.hdr", "High dynamic range"), Settings.HighDynamicRange), ]);

        if (!ModernMenuAvailable)
        {
            rows.Add(MenuItem.Label(Text.Say( "display.modernmenu.missing", "The port's own title screen needs Reborn.rebarn beside the game.")));
        }

        if (Settings.HighDynamicRange)
        {
            // Kept, because it is the one row here that is not a preference: the display either gave back the colour space or it did not, and a.
            rows.Add(MenuItem.Label(HighDynamicRangeActive ? Text.Say("display.hdr.took", "The display took it.") : Text.Say( "display.hdr.refused",
                    "Asked for, and this display did not offer it.")));

            rows.Add(MenuItem.Choice( "transfer", Text.Say("display.transfer", "Encoding"), Describe(Settings.HdrTransfer)));

            rows.Add(MenuItem.Slider( "paperwhite", Text.Say("display.paperwhite", "Paper white"), Fraction(Settings.PaperWhiteNits, 80f, 400f),
                Nits(Settings.PaperWhiteNits)));

            rows.Add(MenuItem.Slider( "peak", Text.Say("display.peak", "Brightest the display goes"), Fraction(Settings.PeakNits, 400f, 4000f),
                Nits(Settings.PeakNits)));

            rows.Add(MenuItem.Slider( "sun", Text.Say("display.sun", "Sunlight"), Fraction(Settings.SunNits, 200f, 4000f), Nits(Settings.SunNits)));

            rows.Add(MenuItem.Slider( "lights", Text.Say("display.lights", "Lamps and windows"), Fraction(Settings.LightNits, 200f, 4000f),
                Nits(Settings.LightNits)));
        }
        else
        {
            rows.Add(MenuItem.Choice( "tonemap", Text.Say("display.tonemap", "Tone curve"), Describe(Settings.ToneMapping)));
        }

        return rows;
    }

    /// <summary>Drawing the room smaller than the window and enlarging it.</summary>
    private List<MenuItem> Upscaling()
    {
        RuntimeFiles files = Runtimes?.For(Settings.Upscaler) ?? UpscalerRuntimes.Unknown(Settings.Upscaler);

        List<MenuItem> rows =
        [
            MenuItem.Heading(Text.Say("picture.upscaling", "Upscaling")), MenuItem.Choice( "upscaler", Text.Say("picture.upscaler", "Upscaler"),
                Describe(Settings.Upscaler)), ];

        if (Settings.Upscaler is UpscalerKind.Fsr or UpscalerKind.Dlss && !files.Present)
        {
            rows.Add(MenuItem.Label(Text.Say( "picture.upscaler.missing", "Not installed: copy {0} into the game's libs folder.",
                List(files.Missing))));
        }
        else if (Settings.Upscaler == UpscalerKind.Dlss && !DlssAvailable)
        {
            // Installed and refused, which is a different sentence: there is nothing to download and nothing the player did wrong.
            rows.Add(MenuItem.Label(Text.Say( "picture.upscaler.wrongcard", "Installed, and this card cannot run it: DLSS needs a GeForce RTX.")));
        }
        ...
        if (Settings.Upscaler != UpscalerKind.Off)
        {
            ...
            rows.Add(MenuItem.Choice( "ratio", Text.Say("picture.ratio", "Quality"), Describe(Settings.UpscalerQuality)) with
            {
                Enabled = !pinned,
            });

            // What the player cannot find out by trying it: the row is dead because the network will not scale, not because the setting stopped.
            if (pinned)
            {
                rows.Add(MenuItem.Label(Text.Say( "picture.ratio.pinned", "Neural uplift draws at the window's own size; it reworks the picture "
                    + "rather than enlarging it.")));
            }

            rows.Add(MenuItem.Label(Between(Settings.Upscaling)));

            rows.Add(Toggle( "sharpen", Text.Say("picture.sharpen", "Sharpen"), Settings.Sharpening));

            if (Settings.Sharpening)
            {
                rows.Add(MenuItem.Slider( "sharpness", Text.Say("picture.sharpness", "How much"), Settings.Sharpness,
        ...
        {
            rows.Add(MenuItem.Choice( "preset", Text.Say("picture.preset", "Model"), DescribePreset(Settings.DlssPreset)));

            rows.Add(Toggle( "reconstruction", Text.Say("picture.reconstruction", "Ray reconstruction"), Settings.RayReconstruction) with
            {
                Enabled = DlssRayReconstruction,
            });

            // Only when it cannot be had.
            if (!DlssRayReconstruction)
            {
                rows.Add(MenuItem.Label( DlssRayReconstructionNote is { Length: > 0 } why
                        ? Text.Say("picture.reconstruction.no", "Not available: {0}.", why) : Text.Say( "picture.reconstruction.missing",
        ...
        {
            UpscalerKind.Fsr => Runtimes?.Fsr.Present ?? false, UpscalerKind.Dlss => DlssFrameGeneration, _ => false,
        };

        rows.Add(MenuItem.Choice( "generation", Text.Say("picture.generation", "Frame generation"), DescribeGeneration(Settings.FrameGeneration)) with
        {
            Enabled = generation,
        });
        ...
        if (!generation)
        {
            rows.Add(MenuItem.Label(Text.Say( "picture.generation.missing",
                "Needs FSR or DLSS, and their frame-generation runtime, in the libs folder.")));
        }
        // No line of its own, and neither does the row above.
        rows.Add(MenuItem.Choice( "latency", Text.Say("picture.latency", "Low latency"), Describe(Settings.Latency)) with
        {
        ...
        {
            rows.Add(MenuItem.Label( Text.Say("picture.running", "Running: {0}", UpscalerRunning)));
        }
    ...
    private List<MenuItem> Neural()
    {
        ...
        List<MenuItem> rows =
        [
            Toggle( "neural", Text.Say("picture.neural", "Neural uplift"), Settings.NeuralUplift) with
            {
                Enabled = installed,
            }, ];
        ...
        if (!installed)
        {
            rows.Add(MenuItem.Label(Text.Say( "picture.neural.missing", "Needs nvngx_dlssnr.dll in the game's libs folder.")));
        ...
        rows.Add(MenuItem.Slider( "nrstrength", Text.Say("picture.nrstrength", "Strength"), Settings.NeuralIntensity,
            MenuPage.Percent(Settings.NeuralIntensity)));

        rows.Add(MenuItem.Slider( "nrtone", Text.Say("picture.nrtone", "Local contrast"), Settings.NeuralLocalTone,
            MenuPage.Percent(Settings.NeuralLocalTone)));

        rows.Add(MenuItem.Slider( "nrglobal", Text.Say("picture.nrglobal", "Overall tone"), Settings.NeuralGlobalTone,
            MenuPage.Percent(Settings.NeuralGlobalTone)));

        rows.Add(MenuItem.Slider( "nrstructure", Text.Say("picture.nrstructure", "Fine detail"), Settings.NeuralLocalStructure,
            MenuPage.Percent(Settings.NeuralLocalStructure)));

        rows.Add(Toggle( "nrskinfollow", Text.Say("picture.nrskinfollow", "Skin follows detail"), Settings.NeuralSkinFollowsStructure));

        if (!Settings.NeuralSkinFollowsStructure)
        {
            rows.Add(MenuItem.Slider( "nrskin", Text.Say("picture.nrskin", "Skin detail"), Settings.NeuralSkinStructure,
                MenuPage.Percent(Settings.NeuralSkinStructure)));
        }

        rows.Add(Toggle( "nrskinmask", Text.Say("picture.nrskinmask", "Find skin"), Settings.NeuralAutoSkinMask));

        rows.Add(MenuItem.Choice( "nrpreset", Text.Say("picture.nrpreset", "Network"), DescribeNetwork(Settings.NeuralPreset)));

        rows.Add(MenuItem.Choice( "nrstyle", Text.Say("picture.nrstyle", "Look"), DescribeNetwork(Settings.NeuralStyle)));

        // What the player cannot find out by trying it: a network that ships one set of weights answers both of those rows with the same picture.
        rows.Add(MenuItem.Label(Text.Say( "picture.neural.oneweight",
    ...
    [
        ...
        MenuItem.Slider( "effects", Text.Say("sound.effects", "Effects"), Settings.EffectsVolume, MenuPage.Percent(Settings.EffectsVolume)),

        MenuItem.Slider( "dialogue", Text.Say("sound.dialogue", "Speech"), Settings.DialogueVolume, MenuPage.Percent(Settings.DialogueVolume)),

        MenuItem.Choice("speakers", Text.Say("sound.speakers", "Speakers"), Describe(Settings.Speakers)),

        // Said rather than quietly not done.
        MenuItem.Label(Text.Say("sound.speakers.note", "Speakers take effect at the next start.")), ];

    /// <summary>How the game plays, and the things it will do for the player rather than ask of them.</summary>
    private IReadOnlyList<MenuItem> Gameplay() =>
    [
        // First, and on this page rather than on Sound, because it is not a preference about how the game sounds — it decides what is said, what is.
        MenuItem.Choice("language", Text.Say("general.language", "Language"), Describe(Language)),

        Languages.Count > 1 ? MenuItem.Label(Text.Say( "general.language.note", "Language takes effect at the next start."))
            : MenuItem.Label(Text.Say( "general.language.only", "Only English is installed. Other languages need their own pack beside "
                + "the game.")),

        MenuItem.Slider( "hurry", Text.Say("general.hurry", "Hurrying pace"), (Settings.HurryFactor - 1f) / 3f,
            string.Create(CultureInfo.InvariantCulture, $"{Settings.HurryFactor:F1}x")),

        Toggle( "glide", Text.Say("general.glide", "Camera travels between angles"), Settings.CameraGlide),
        ...
        MenuItem.Slider( "walkpace", Text.Say("general.walkpace", "Walking pace"), Fraction( Settings.FirstPersonSpeed,
                GK3Reborn.Game.Navigation.FirstPerson.SlowestPace, GK3Reborn.Game.Navigation.FirstPerson.FastestPace), MenuPage.Percent(Fraction(
                Settings.FirstPersonSpeed, GK3Reborn.Game.Navigation.FirstPerson.SlowestPace,
                GK3Reborn.Game.Navigation.FirstPerson.FastestPace))) with
        {
            Enabled = Settings.FirstPerson,
        },

        MenuItem.Slider( "looksense", Text.Say("general.looksense", "Looking about"), Fraction( Settings.LookSensitivity,
                GK3Reborn.Game.Navigation.FirstPerson.SlowestLook, GK3Reborn.Game.Navigation.FirstPerson.FastestLook),
            Times(Settings.LookSensitivity)) with
        {
            Enabled = Settings.FirstPerson,
        },

        Toggle( "invertlook", Text.Say("general.invertlook", "Invert looking up and down"), Settings.InvertLook) with
        {
            Enabled = Settings.FirstPerson,
        },
        ...
        Toggle( "filmcaptions", Text.Say("general.filmcaptions", "Write out what is said in films"), Settings.MovieSubtitles),

        Toggle( "intro", Text.Say("general.intro", "Play the intro on starting"), Settings.PlayIntro),

        Toggle("eggs", Text.Say("general.eggs", "Easter eggs"), Settings.EasterEggs),

        // Named for what it gives rather than for what it is.
        MenuItem.Choice( "restored", Text.Say("general.restored", "Cut content"), Describe(Settings.RestoredContent)),

        MenuItem.Heading(Text.Say("general.easier", "Made easier")),

        Toggle( "moustache", Text.Say("general.moustache", "Skip the cat-hair moustache"), Settings.AlwaysWearsMoustache),

        Toggle( "armour", Text.Say("general.armour", "Gabriel cannot be killed"), Settings.PlotArmour),

        Toggle( "catch", Text.Say("general.catch", "Gabriel catches the pendulum"), Settings.CatchesPendulum), ];
    ...
    private List<MenuItem> Controls()
    {
        ...
        [
            ...
            MenuItem.Slider( "cursorsize", Text.Say("controls.cursorsize", "Pointer size"), Fraction( Settings.CursorScale,
                    GK3Reborn.Game.Settings.SmallestCursor, GK3Reborn.Game.Settings.LargestCursor), Times(Settings.CursorScale)),

            MenuItem.Heading(Text.Say("controls.gamepad", "Gamepad")),

            Toggle( "padcursor", Text.Say("controls.padcursor", "Left stick moves the pointer"), Settings.GamepadCursor),

            MenuItem.Slider( "padspeed", Text.Say("controls.padspeed", "How fast"), Fraction( Settings.GamepadCursorSpeed,
                    GK3Reborn.Game.Settings.SlowestCursor, GK3Reborn.Game.Settings.FastestCursor), MenuPage.Percent(Fraction(
                    Settings.GamepadCursorSpeed, GK3Reborn.Game.Settings.SlowestCursor, GK3Reborn.Game.Settings.FastestCursor))) with
            {
        ...
        if (!HasGamepad)
        {
            rows.Add(MenuItem.Label(Text.Say( "controls.nopad", "No gamepad is plugged in. These can still be set.")));
        }
        ...
        {
            rows.Add(MenuItem.Binding( "ptr:" + pointer, Named(pointer), Waiting("ptr:" + pointer)
                    ? Text.Say("controls.pressbutton", "Press a button…") : Named(bound.Button(pointer))));
        }
        ...
        {
            rows.Add(MenuItem.Binding( "key:" + action, Named(action), Waiting("key:" + action) ? Text.Say("controls.presskey", "Press a key…")
                    : Bound(bound, action)));
        }
        ...
        foreach (CameraAction action in InputBindings.Actions)
        {
            rows.Add(MenuItem.Binding( "pad:" + action, Named(action), Waiting("pad:" + action) ? Text.Say("controls.pressbutton", "Press a button…")
                    : Named(bound.Button(action))));
        }

        rows.Add(MenuItem.Button( "bindreset", Text.Say("controls.reset", "Put every control back")));

        // What the player cannot find out by trying it: which way out of a rebind there is, and that there is one at all.
        if (Listening)
        {
            rows.Add(MenuItem.Label(Text.Say( "controls.listening", "Escape leaves it alone. Backspace clears it.")));
        }

        return rows;
    }

    /// <summary>The bindings as they now stand, read back out of the settings.</summary>
    ...
    private bool Waiting(string id) => string.Equals(_listening, id, StringComparison.Ordinal);
    ...
    /// <summary>Binds whatever the player just pressed to whatever they were rebinding.</summary>
    /// <returns>True when something was bound and the page should be redrawn.</returns>
    /// <param name="key">The key pressed, or for none.</param>
    ...
    private void Change(MenuAction action)
    {
        ...
        Settings = action.Id switch
        {
            ...
            // Through what is installed rather than through everything GK3 was published in, so a player with English and French steps between two.
            ...
            "window" => Settings with { Display = Step(Windows, Settings.Display, action.Step) }, "size" => Size(action.Step),
            ...
            {
                Upscaler = Step( Offered.Count > 0 ? [.. Offered] : EveryUpscaler, Settings.Upscaler, action.Step),
            },
            ...
            {
                NeuralPreset = Wrapped( Settings.NeuralPreset + (action.Step == 0 ? 1 : action.Step), NeuralUplift.Highest + 1),
            },

            "nrstyle" => Settings with
            {
                NeuralStyle = Wrapped( Settings.NeuralStyle + (action.Step == 0 ? 1 : action.Step), NeuralUplift.Highest + 1),
            },

            // Round the letters rather than stopping at the ends, the same way every other choice on these pages does, and past the ones with names.
            "preset" => Settings with
            {
                DlssPreset = Wrapped( Settings.DlssPreset + (action.Step == 0 ? 1 : action.Step), DlssPresets.Highest + 1),
            },
            ...
            // Takes effect the next time the menu is opened, which for the row itself is the next start: the screen it changes is the one the player.
            "modernmenu" => Settings with { ModernMenu = !Settings.ModernMenu },

            "enhanced" => Settings with { EnhancedTextures = !Settings.EnhancedTextures },
            "trees" => Settings with { ModelledTrees = !Settings.ModelledTrees }, "grass" => Settings with { Grass = !Settings.Grass },
            ...
            "rails" => Settings with { ThickCutoutCards = !Settings.ThickCutoutCards }, "birds" => Settings with { Birds = !Settings.Birds },
            ...
            {
                FirstPersonSpeed = MathF.Round(Between( Settings.FirstPersonSpeed, GK3Reborn.Game.Navigation.FirstPerson.SlowestPace,
                    GK3Reborn.Game.Navigation.FirstPerson.FastestPace, action)),
            },

            "looksense" => Settings with
            {
                LookSensitivity = MathF.Round( Between( Settings.LookSensitivity, GK3Reborn.Game.Navigation.FirstPerson.SlowestLook,
                        GK3Reborn.Game.Navigation.FirstPerson.FastestLook, action) * 20f) / 20f,
            }, "captions" => Settings with { Captions = !Settings.Captions }, "intro" => Settings with { PlayIntro = !Settings.PlayIntro },
            ...
            "restored" => Settings with
            {
                ...
                {
                    CutContentTier.None => CutContentTier.Observation, CutContentTier.Observation => CutContentTier.All,
                    CutContentTier.All => CutContentTier.Reconstructed, _ => CutContentTier.None,
                },
            },
            ...
            "armour" => Settings with { PlotArmour = !Settings.PlotArmour }, "catch" => Settings with { CatchesPendulum = !Settings.CatchesPendulum },
            ...
            {
                Reflectivity = GK3Reborn.Game.Settings.MostReflective * Level(Settings.Reflectivity / GK3Reborn.Game.Settings.MostReflective, action),
            },

            "padcursor" => Settings with { GamepadCursor = !Settings.GamepadCursor },

            // Rounded to a twentieth so the row reads as a round number and two players who set it to the same thing get the same thing.
            "cursorsize" => Settings with
            {
                CursorScale = MathF.Round( Between( Settings.CursorScale, GK3Reborn.Game.Settings.SmallestCursor,
                        GK3Reborn.Game.Settings.LargestCursor, action) * 20f) / 20f,
            },

            "padspeed" => Settings with
            {
                GamepadCursorSpeed = Between( Settings.GamepadCursorSpeed, GK3Reborn.Game.Settings.SlowestCursor,
                    GK3Reborn.Game.Settings.FastestCursor, action),
            },
    ...
    {
        int at = Array.FindIndex( Sizes, s => s.Width == Settings.DisplayWidth && s.Height == Settings.DisplayHeight);

        int next = Wrapped((at < 0 ? 0 : at) + (by == 0 ? 1 : by), Sizes.Length);

        return Settings with
        {
            DisplayWidth = Sizes[next].Width, DisplayHeight = Sizes[next].Height,
        };
    }
    ...
    private static float TextSize(float current, MenuAction action)
    {
        ...
        return MathF.Round( (SmallestText + ((LargestText - SmallestText) * part)) * 20f) / 20f;
    }

    /// <summary>How the text-size row reads.</summary>
    private string DescribeTextScale() => string.Create( CultureInfo.InvariantCulture, $"{Settings.TextScale * 100f:F0}%");

    /// <summary>How this page reads the resolution row.</summary>
    private string DescribeSize() => Settings.Display == WindowMode.BorderlessFullscreen || Settings.DisplayWidth <= 0 || Settings.DisplayHeight <= 0
            ? Text.Say("display.size.monitor", "The monitor's own") : string.Create( CultureInfo.InvariantCulture,
                $"{Settings.DisplayWidth}x{Settings.DisplayHeight}");

    /// <summary>Where a luminance slider sits between its two ends.</summary>
    private static float Fraction(float value, float low, float high) => Math.Clamp((value - low) / MathF.Max(high - low, 1f), 0f, 1f);
    ...
    private static string Times(float value) => string.Create( CultureInfo.InvariantCulture, $"{value * 100:F0}%");

    /// <summary>How a luminance reads.</summary>
    private string Nits(float value) => Text.Say( "display.nits", "{0} nits", value.ToString("F0", CultureInfo.InvariantCulture));
    ...
    {
        0 => Text.Say("picture.upscaler.nothing", "nothing"), 1 => names[0],
        _ => string.Join(Text.Say("list.comma", ", "), names.Take(names.Count - 1)) + Text.Say("list.and", " and ") + names[^1],
    };

    /// <summary>Where a slider ends up: dragged outright, or stepped a twentieth.</summary>
    private static float Level(float current, MenuAction action) => Math.Clamp( action.Dragged ? action.Fraction : current + (action.Step * 0.05f),
            0f, 1f);

    /// <summary>The next one round the list, either way.</summary>
    private static T Step<T>(T[] all, T current, int by) where T : struct, Enum
    {
    ...
    private static T Next<T>(IReadOnlyList<T> all, T current, int by) where T : class
    {
        if (all.Count == 0)
        {
            return current;
    ...
    /// <summary>The two sizes the picture is drawn between.</summary>
    ...
    private string Between(UpscalePlan plan)
    {
        ...
        return both.Split(" to ") is [string drawn, string shown] ? Text.Say("picture.upscaler.sizes", "{0} to {1}", drawn, shown) : both;
    }

    /// <summary>What one of NVIDIA's presets is called on the page.</summary>
    ...
    private string DescribePreset(int preset)
    {
        ...
        string letter = string.Create( CultureInfo.InvariantCulture, $"{(char)('A' + preset - 1)}");

        string note = preset switch
        {
            10 => Text.Say("picture.preset.transformer", " (transformer)"), 11 => Text.Say("picture.preset.best", " (transformer, best picture)"),
            12 => Text.Say("picture.preset.steadiest", " (transformer, steadiest)"),
            13 => Text.Say("picture.preset.fastest", " (transformer, fastest)"), _ => string.Empty,
        };
    ...
    private string DescribeNetwork(int ordinal) => ordinal <= 0 ? Text.Say("picture.nrpreset.network", "Whatever the network prefers") : Text.Say(
            "picture.nrpreset.number", "Number {0}", ordinal.ToString(CultureInfo.InvariantCulture));
    ...
    private string DescribeGeneration(FrameGeneration generation) => generation == FrameGeneration.Off ? Text.Say("value.off", "Off")
            : generation.Describe();

    /// <summary>What one of the game's actions is called on the Controls page.</summary>
    /// <returns>Its name.</returns>
    /// <param name="action">The action.</param>
    private string Named(CameraAction action) => Text.Say("action." + action, InputBindings.Name(action));

    /// <summary>What clicking a mouse button means in this game.</summary>
    private string Named(PointerButton button) => Text.Say("pointer." + button, InputBindings.Name(button));

    /// <summary>Where a gamepad button is on the pad.</summary>
    private string Named(GamepadButton button) => Text.Say("pad." + button, GamepadButtons.Describe(button));

    /// <summary>Which keys an action is bound to.</summary>
    ...
    private string Bound(InputBindings bound, CameraAction action)
    {
        ...
        return keys.Count == 0 ? "—" : string.Join( Text.Say("list.comma", ", "),
    ...
    private string Describe(CutContentTier tier) => tier switch
    {
        ...
        CutContentTier.All => Text.Say("general.restored.all", "Everything, puzzles included"), CutContentTier.Reconstructed =>
            Text.Say("general.restored.rebuilt", "And objects rebuilt from scratch"), _ => Text.Say("value.off", "Off"),
    };
    ...
    private string Describe(PictureQuality quality) => quality switch
    {
        ...
        PictureQuality.High => Text.Say("picture.lighting.high", "Shadows and shading"), _ => Text.Say("picture.lighting.everything", "Everything"),
    };

    private string Describe(WindowMode mode) => mode switch
    {
        WindowMode.BorderlessFullscreen => Text.Say("display.window.borderless", "Borderless, filling the monitor"),
        WindowMode.ExclusiveFullscreen => Text.Say("display.window.full", "Fullscreen"), _ => Text.Say("display.window.windowed", "A window"),
    };

    // The two vendors' names are names.
    private string Describe(UpscalerKind kind) => kind switch
    {
        UpscalerKind.Spatial => Text.Say("picture.upscaler.builtin", "Built in"), UpscalerKind.Fsr => "FSR (AMD)",
        UpscalerKind.Dlss => "DLSS (NVIDIA)", _ => Text.Say("value.off", "Off"),
    };
    ...
    private string DescribeBackend()
    {
        ...
        string name = Settings.Backend == RenderBackend.Automatic ? Text.Say("display.backend.automatic", "Automatic ({0})", Describe(chosen))
            : Describe(chosen);

        return RunningBackend != RenderBackend.Automatic && RunningBackend != chosen ? name + Text.Say("display.backend.nextstart", ", next start")
            : name;
    }

    private static string Describe(RenderBackend backend) => backend switch
    {
        RenderBackend.Direct3D12 => "Direct3D 12", _ => "Vulkan",
    };

    private string Describe(LatencyMode latency) => latency switch
    {
        LatencyMode.On => Text.Say("value.on", "On"), LatencyMode.Boost => Text.Say("picture.latency.boost", "On + boost"),
    ...
    {
        HdrTransfer.PerceptualQuantiser => "HDR10", HdrTransfer.ExtendedLinear => "scRGB",
        _ => Text.Say("display.transfer.either", "Whichever the display prefers"),
    };

    private string Describe(ToneMapping curve) => curve switch
    {
        ToneMapping.Reinhard => Text.Say("display.tonemap.rolled", "Rolled off"), ToneMapping.Filmic => Text.Say("display.tonemap.filmic", "Filmic"),
    ...
    private string Describe(SpeakerLayout layout) => layout switch
    {
        ...
        SpeakerLayout.Surround51 => Text.Say("sound.speakers.surround", "Surround, 5.1"), _ => Text.Say("sound.speakers.stereo", "Stereo"),
    };
}
